...
function getJWTSecret() {
    ...
    if (exports.config.env.isProduction && exports.config.jwt.secret === 'development-secret-change-in-production') {
        throw new Error('JWT_SECRET must be changed in production');
    }
    return exports.config.jwt.secret;
}
/**
...
function validateConfig() {
    // Validate port
    if (isNaN(exports.config.server.port) || exports.config.server.port < 1 || exports.config.server.port > 65535) {
        throw new Error(`Invalid PORT: ${process.env.PORT}`);
    }
    // Validate rate limits
    if (exports.config.rateLimit.global.max < 1) {
        throw new Error(`Invalid GLOBAL_RATE_LIMIT: ${process.env.GLOBAL_RATE_LIMIT}`);
    }
    if (exports.config.rateLimit.auth.max < 1) {
        throw new Error(`Invalid AUTH_RATE_LIMIT: ${process.env.AUTH_RATE_LIMIT}`);
    }
    // Validate log level
    const validLogLevels = ['debug', 'info', 'warn', 'error'];
    if (!validLogLevels.includes(exports.config.logging.level)) {
        throw new Error(`Invalid LOG_LEVEL: ${process.env.LOG_LEVEL}`);
    }
    // Warn about development secrets in production
    if (exports.config.env.isProduction) {
        if (exports.config.jwt.secret === 'development-secret-change-in-production') {
